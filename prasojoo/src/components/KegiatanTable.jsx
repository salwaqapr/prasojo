...
export default function KegiatanTable({ data, onEdit, onDelete, sortBy, sortDir, onToggleSort }) {
  return (
      ...
      <table className="min-w-[1000px] w-full bg-white">
        <thead className="bg-[#111827] text-white text-sm">
          <tr>
            ...
            <th className="p-2 text-center cursor-pointer" onClick={onToggleSort}>
              Tanggal{" "}
              {sortBy !== "tanggal" && "⇅"}
        ...
        <tbody>
          ...
          {data.map((k, i) => (
            <tr key={k.id} className="border-b border-gray-300 hover:bg-gray-50">
              ...
              <td className="p-2 text-center font-medium">G-{String(k.id).padStart(4, "0")}</td>
              <td className="p-2 text-center font-medium">{k.tanggal.split("-").reverse().join("-")}</td>
              <td className="p-2 font-medium">{k.judul}</td>
              <td className="p-2 font-medium">{k.deskripsi}</td>

...
@section('content')
@php
    $canManage = in_array(auth()->user()->nama, ['Admin', 'Sie Inventaris']);

    $totalPemasukan = 0;
    $totalPengeluaran = 0;
    foreach ($inventaris as $i) {
        $totalPemasukan += $i->pemasukan ?? 0;
        $totalPengeluaran += $i->pengeluaran ?? 0;
    }
...
<div class="p-3">
    ...
    <div class="responsive mt-6">
        ...
        <table class="min-w-[900px] w-full bg-white">
            ...
            <tbody class="text-sm" id="inventarisTable">
            ...
            <tr data-id="{{ $item->id }}" class="border-b hover:bg-gray-50">
                ...
                <td class="p-2 text-center whitespace-nowrap font-medium">
                    I-{{ str_pad($item->id, 4, '0', STR_PAD_LEFT) }}
                </td>
                <td class="p-2 text-center whitespace-nowrap font-medium">
...
btnTambah.onclick = ()=>{
    editingId = null;
    modalTitle.innerText = "Tambah Data Inventaris";
    kosongkanForm();
    hideError();
...
document.addEventListener("click", e => {
    ...
    if (editBtn) {
        ...
        modalTitle.innerText = "Edit Data Inventaris";
...
hapusYa.onclick = async ()=> {
    const res = await fetch(`/inventaris/${rowToDelete.dataset.id}`, {
        method:"DELETE",
...
function resetNomor() {
    let no = 1;
    document.querySelectorAll("#inventarisTable tr").forEach(row => {
...
function collectTanggalData() {
    dataTanggal = [];
    const rows = document.querySelectorAll("#inventarisTable tr");

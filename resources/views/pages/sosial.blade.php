            ...
            <tbody id="sosialTable">
                ...
                @foreach ($sosial as $index => $item)
                    ...
                    <tr data-id="{{ $item->id }}">
                        ...
                        <td class="border p-2 text-center">S-{{ str_pad($item->id, 4, '0', STR_PAD_LEFT) }}</td>
...
document.getElementById("btnTambah").onclick = ()=>{
    ...
    kosongkanForm();
    hideError();
    document.getElementById("modalTitle").innerText = "Tambah Sosial";
...
document.addEventListener("click", function(e){
    if(e.target.classList.contains("editBtn")){
        ...
        document.getElementById("modalTitle").innerText = "Edit Sosial";
...
document.getElementById("hapusYa").onclick = async ()=>{
    ...
    const res = await fetch(`/sosial/${id}`,{
        method:"DELETE",
